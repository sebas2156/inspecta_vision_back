...
from shared.models import Camaras, Regiones, productosector, Productos, Suscripciones, ModelosIa, Logs
                        ...
                        for poly_idx, (poly, restrictions) in enumerate(zip(polygons, cam.restricciones)):
                            if punto_en_poligono((cx, cy), poly):
                                ...
                                if detected_class in restrictions:
                                    ...
                                    print(
                                        f"ALERTA [{self.model_id}]: {detected_class} en cámara {cam_id}, sector {sector_id}")
...
def obtener_configuracion_camaras(db: Session):
    try:
        resultados = db.query(
            Camaras.id,
            Camaras.ip,
            Suscripciones.modelo_id,
            Regiones.coordenadas,
            productosector.stock_minimo,
            productosector.stock_maximo,
            ...
            .join(productosector, productosector.sector_id == Regiones.sector_id) \
            .join(Productos, (Productos.codigo == productosector.producto_codigo) &
                  (Productos.empresa_id == productosector.empresa_id)) \
        ...
        for row in resultados:
            ...
            # Configuración basada en productos
            producto_config = {
            ...
            config[modelo_id][cam_id].append({
                "ip": row.ip,
                "polygons": polygons,
                "productos": [producto_config],
                "sector_id": row.sector_id
            })

        lista_final = []
        for modelo_id, camaras_data in config.items():
            dispositivos = []
            for cam_id, configs in camaras_data.items():
                combined = {
                    "ip": configs[0]["ip"],
                    "polygons": [],
                ...
                for cfg in configs:
                    combined["polygons"].extend(cfg["polygons"])
                    combined["sectores"].append(cfg["sector_id"])

                    for producto in cfg["productos"]:
                        combined["productos"][producto["codigo"]] = {
                            "stock_min": producto["stock_min"],
                            "stock_max": producto["stock_max"],
                            "permitido": producto["permitido"]
                        }
                ...
                dispositivos.append({
                    ...
                    "productos": dict(combined["productos"]),
                    "sectores": combined["sectores"]
                })
